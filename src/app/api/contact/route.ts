...
export const POST = withRateLimit(
  ...
  async (request: Request) => {
    try {
      ...
      // Send email notification to admin. This one is required — if the
      // operator doesn't get notified, the submission is effectively lost
      // (it's in the DB but won't be seen). Surface failure to the user as a
      // 500 instead of pretending the message went through. The DB row stays
      // so the operator can still recover it from the admin UI.
      ...
      try {
        ...
        // Do not echo the underlying error to the client (info disclosure).
        return NextResponse.json(
          {
            error:
              "We received your message but had trouble notifying the recipient. Please try again in a few minutes.",
          },
          { status: 500 },
        );
      }
      ...
      try {
        ...
        console.error(
          "Auto-reply email failed (non-fatal):",
          err instanceof Error ? err.message : err,
        );
      }
...
function generatePersonalizedResponse(
  ...
  if (
    ...
    lowerSubject.includes("collaboration") ||
    lowerSubject.includes("work together") ||
