...
# Otherwise it forwards to the app directly (legacy HTTP-only behavior).
...
# ALB Listener HTTPS - only created when acm_certificate_arn is supplied and
# the stack is not paused. Operator requests the certificate out-of-band
# (`aws acm request-certificate --domain-name <yourdomain> --validation-method DNS`),
# validates it, and drops the ARN into terraform.tfvars.
...
# ECR Lifecycle Policy (keep last 10 images for prod)
